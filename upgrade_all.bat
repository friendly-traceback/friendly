call venv-friendly-3.6\scripts\activate
call python -m pip install friendly-traceback --upgrade
call python -m pip install -r requirements.txt --upgrade

call venv-friendly-3.7\scripts\activate
call python -m pip install friendly-traceback --upgrade
call python -m pip install -r requirements.txt --upgrade


call venv-friendly-3.8\scripts\activate
call python -m pip install friendly-traceback --upgrade
call python -m pip install -r requirements.txt --upgrade


call venv-friendly-3.9\scripts\activate
call python -m pip install friendly-traceback --upgrade
call python -m pip install -r requirements.txt --upgrade

call venv-friendly-3.10\scripts\activate
call python -m pip install friendly-traceback --upgrade
call python -m pip install -r requirements.txt --upgrade

call venv-friendly-ipython\scripts\activate
call python -m pip install friendly-traceback --upgrade
call python -m pip install -r requirements.txt --upgrade

call venv-friendly-3.9\scripts\activate
